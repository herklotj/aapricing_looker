...
JOIN dbuser.qs_cover c ON rr.quote_id = c.quote_id AND to_date(SYSDATE) - to_date(rr.quote_dttm) <= 7 AND to_date(SYSDATE) - to_date(rr.quote_dttm) >= 0

INNER JOIN dbuser.qs_drivers d1 on rr.quote_id = d1.quote_id AND d1.driver_id = 0
LEFT JOIN dbuser.qs_mi_outputs mi ON rr.quote_id = mi.quote_id

LEFT JOIN dbuser.qs_drivers d2 ON rr.quote_id = d2.quote_id AND d2.driver_id = 1
LEFT JOIN dbuser.qs_drivers d3 ON rr.quote_id = d3.quote_id AND d3.driver_id = 2
LEFT JOIN dbuser.qs_drivers d4 ON rr.quote_id = d4.quote_id AND d4.driver_id = 3
